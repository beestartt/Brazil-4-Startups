...
function submitNewLead() {
    ...
    const url = "https://us-central1-mailer-8c155.cloudfunctions.net/sendEmailMailGun";
    Http.open("POST", url);
    ...
    const parameters = JSON.stringify({
        "from": "hello@brazil4startups.com",
        "to": "hello@brazil4startups.com",
        "subject": "Nova submissão de lead",
        "html": "<p>Nome: "+ name + "</p><p>Email: "+email+"</p>"
    });

    Http.send(parameters);

    Http.onreadystatechange = e => {
        var sucess = JSON.parse(Http.responseText).success;

        if (sucess) {
            alert("Thank you for your submission. I'll be contacted soon!")
        } else {
            alert("Some error has occoured. Please try again later");
        }
    };
}
